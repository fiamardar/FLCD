class SymbolTable:
    ...
    def search(self, element_to_search):
        ...
        :return: hash_code - the hash code of the element in case it is the only one associated to it
        ...
        if hash_code not in self._elements.values():
            return False
        else:
            ...
            if type(result) is list:
                ...
                # Else, check if the element associated to the key is the one we are searching for
    ...
    def add(self, element_to_add):
        ...
        if hash_code not in self._elements.keys():
            ...
            if type(result) is list:
                ...
                # If there is only one element associated to the hash code, check if it is the one we want to add
                if result == element_to_add:
                    # And return the key (the hash code)
                    return hash_code
                else:
                    # Else, transform the value into a list containing both elements
                    self._elements[hash_code] = [result, element_to_add]
                    # And return the tuple of key and index
                    return hash_code, result.index(element_to_add)
    ...
    def delete(self, element_to_delete):
        ...
        if hash_code in self._elements.keys():
            ...
            if type(result) is list:
                ...
                # If there is only one element associated to the hash code, check if it is the one we want to delete
                if result == element_to_delete:
                    # Then delete it and return the hash code
                    del result[element_to_delete]
                    return hash_code
